...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(cols, rows, local_conf, func) {
    return session.handleCreate(cols, rows, local_conf, func);
  });
...
function FileView() { return this; }
FileView.prototype.ls = function(path, func) {
    var lst;
    try {
...
FileView.prototype.save = function(filename, contents, func) {
...
FileView.prototype.execute = function(script, func) {
    try {
...
Session.prototype.handleCreate = function(cols, rows, local_conf, func) {
  var self = this
    , terms = this.terms
  ...
  term = pty.spawn(shell, shellArgs, {
    ...
    cols: cols,
    rows: rows,
    cwd: fs.realpathSync(local_conf.cwd || conf.cwd || process.env.HOME)
  });
