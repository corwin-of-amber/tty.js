...
Server.prototype.initMiddleware = function() {
  ...
  this.use(express.static(__dirname + '/../node_modules/jquery/dist'));
  this.use(express.static(__dirname + '/../node_modules/underscore'));
  this.use(express.static(__dirname + '/../node_modules/backbone'));
  this.use('/codemirror', express.static(__dirname + '/../node_modules/codemirror'));

  if (conf.static) {
    var staticDirs = typeof conf.static === 'string' ? [conf.static] : conf.static;
    staticDirs.forEach(dir => {
      this.use(express.static(untildify(dir)));
    });
  }
...
Server.prototype.handleOptions = function(req, res, next) {
  ...
  fs.readFile(conf.json, 'utf8', function(err, data) {
    ...
    var term_opts = 
      config.helpers.merge(config.helpers.clone(conf.term), data.term || {});

    res.send('tty.conf = ' + JSON.stringify(data, null, 2) + ';\n'
      + 'Terminal._opts = ' + JSON.stringify(term_opts, null, 2) + ';\n'
      + '('
      + applyConfig
      + ')();');
  });
...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(opts, func) {
    return session.handleCreate(opts, func);
  });
  ...
  // Addons
  require('./filesystem')(socket);
  require('./runner')(socket);
};
...
Session.prototype.handleCreate = function(opts, func) {
  var self = this
    , terms = this.terms
  ...
  var shell = opts.shell || conf.shell,
      shellArgs = opts.shellArgs || conf.shellArgs,
      cols = opts.cols || conf.term.geometry[0] || 80,
      rows = opts.rows || conf.term.geometry[1] || 24,
      cwd = untildify(opts.cwd || conf.cwd || '~');

  if (typeof shell === 'function') shell = shell(this);
  if (typeof shellArgs === 'function') shellArgs = shellArgs(this);
